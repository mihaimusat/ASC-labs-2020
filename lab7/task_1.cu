...
int main(void) {
	...
	error = cudaMalloc(&device_a, num_bytes);
	DIE(error != cudaSuccess || device_a == NULL, "cudaMalloc");

	fill_array_int(host_a, num_elements);

	error = cudaMemcpy(device_a, host_a, num_bytes, cudaMemcpyHostToDevice);
	DIE(error != cudaSuccess, "cudaMemcpy");
    	...
    	kernel_parity_id<<<num_elements / 4, 4>>>(device_a, num_elements);

	error = cudaDeviceSynchronize();
	DIE(error != cudaSuccess, "cudaDeviceSynchronize");

	error = cudaMemcpy(host_a, device_a, num_bytes, cudaMemcpyDeviceToHost);
	DIE(error != cudaSuccess, "cudaMemcpy");
	...
	check_task_1(3, host_a);
   	...
   	kernel_block_id<<<num_elements / 4, 4>>>(device_a, num_elements);
    	...
    	kernel_thread_id<<<num_elements / 4, 4>>>(device_a, num_elements);
	...
	DIE(error != cudaSuccess, "cudaFree");
